...
class BankvanbredaPipeline:

    # Database setup
    conn = sqlite3.connect('bankvanbreda.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bankvanbreda`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bankvanbreda WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bankvanbreda (date, title, link, content)"
